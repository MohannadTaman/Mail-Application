    ...
    while 1:
        ...
        if signup_flag == '1':
            ...
            if row is not None:
                ...
                cr.execute(
                    "SELECT sender , subject , message , data , filename FROM maildata where rec=?", (email,))
                rows = cr.fetchall()
                senders = []
                subjects = []
                messages = []
                ...
                if (senders and subjects and messages and datas and filenames):
                    ...
                    connectionSocket.send(bytes("\n".join(subjects), "utf-8"))
                    time.sleep(5)
                    connectionSocket.send(bytes("".join(messages), "utf-8"))
                    time.sleep(5)
                    ...
                    flag_null = '1'
                    connectionSocket.send(bytes(flag_null, "utf-8"))
                flag_content_message = connectionSocket.recv(
                    2048).decode("utf-8")
