...
def upload_attachment():
    ...
    global get_filename
    global get_content
    get_filename = filedialog.askopenfilenames(title="Select Attachment", filetypes=(
        ("png", ".png"), ("jpg", ".jpg"), ("rar", ".rar"), ("txt", ".txt"), ("Allfile", ".")))
    get_content = convert_content_binary(get_filename)
    CTkMessagebox(title="Attachment",
                  message="Attachment Uploaded Successfully", icon="check")
...
def new_email_to_database_without_attach(to, email, subject, message):
    ...
    time.sleep(5)
    CTkMessagebox(
        title="Email", message="Email Sent Successfully", icon="check")
...
def send_email(email):
    ...
    frame = customtkinter.CTkFrame(
        master=NewEmail, width=320, height=360, corner_radius=15)
    frame.place(relx=0.5, rely=0.5, anchor=tkinter.CENTER)

    l2 = customtkinter.CTkLabel(
        master=frame, text="To: ", font=('Century Gothic', 12))
    l2.place(x=20, y=20)

    ToEntry = customtkinter.CTkEntry(master=frame, width=220, placeholder_text='',
                                     fg_color="transparent", border_color="#3b3a3a", border_width=1, corner_radius=20)
    ToEntry.place(x=75, y=20)

    l3 = customtkinter.CTkLabel(
        master=frame, text="From: ", font=('Century Gothic', 12))
    l3.place(x=20, y=70)

    fromm = customtkinter.CTkLabel(
        master=frame, text=email, font=('Century Gothic', 12))
    fromm.place(x=75, y=70)

    l3 = customtkinter.CTkLabel(
        master=frame, text="Subject: ", font=('Century Gothic', 12))
    l3.place(x=20, y=120)

    SubEntry = customtkinter.CTkEntry(master=frame, width=220, placeholder_text='',
                                      fg_color="transparent", border_color="#3b3a3a", border_width=1, corner_radius=20)
    SubEntry.place(x=75, y=120)

    l3 = customtkinter.CTkLabel(
        master=frame, text="Compose Email: ", font=('Century Gothic', 12))
    l3.place(x=20, y=160)

    textbox = customtkinter.CTkTextbox(
        master=frame, height=150, width=285, fg_color="transparent", border_color="#3b3a3a", border_width=1)
    textbox.place(x=15, y=190)

    attachment = ImageTk.PhotoImage(Image.open(
        "Images\\attachment.png").resize((30, 30), Image.ANTIALIAS))
    attachment_btn = customtkinter.CTkButton(master=frame, image=attachment, text="", width=50, height=20,
                                             fg_color="transparent", hover_color="#1e1f1e", compound="left", command=lambda: upload_attachment())
    attachment_btn.place(x=240, y=300)

    send = ImageTk.PhotoImage(Image.open(
        "Images\\send2.png").resize((30, 30), Image.ANTIALIAS))
    send_btn = customtkinter.CTkButton(master=NewEmail, image=send, text="", width=50, height=20, fg_color="transparent", hover_color="#1e1f1e", compound="left", command=lambda: new_email_to_database(ToEntry.get(
    ), email, SubEntry.get(), textbox.get(1.0, END), get_content, get_filename) if get_filename != 'filename' else new_email_to_database_without_attach(ToEntry.get(), email, SubEntry.get(), textbox.get(1.0, END)))
    ...
    if filename != "NULL":
        ...
        if file_path:
            with open(file_path, 'wb') as f:
                ...
                time.sleep(5)
                CTkMessagebox(
                    title="Attachment", message="Attachment Downloaded Successfully", icon="check")
...
def viewemail(sender, subject, message, content, filename):
    ...
    email.title('Email')
    frame = customtkinter.CTkFrame(
        master=email, width=320, height=360, corner_radius=15)
    frame.place(relx=0.5, rely=0.5, anchor=tkinter.CENTER)

    l3 = customtkinter.CTkLabel(
        master=frame, text="From:        " + sender, font=('Century Gothic', 12))
    l3.place(x=20, y=20)

    l3 = customtkinter.CTkLabel(
        master=frame, text="Subject: " + subject, font=('Century Gothic', 12))
    l3.place(x=20, y=70)

    frame2 = customtkinter.CTkFrame(master=email, width=285, height=220,
                                    fg_color="transparent", border_color="#3b3a3a", corner_radius=20)
    frame2.place(x=157, y=160)
    l4 = customtkinter.CTkLabel(master=frame2, height=170, width=285,
                                fg_color="transparent", text="" + message, corner_radius=20, anchor="nw")
    l4.place(x=0, y=10)

    frame3 = customtkinter.CTkFrame(
        master=frame2, width=285, height=35, fg_color="#363636", border_color="#3b3a3a", corner_radius=0)
    frame3.place(x=0, y=190)

    if filename != 'NULL':
        btn5 = customtkinter.CTkLabel(master=frame3, height=20, width=200, fg_color="transparent",
                                      text="Attachment: "+filename, anchor="w", font=('Century Gothic', 12))
        btn5.place(x=5, y=5)

    download = ImageTk.PhotoImage(Image.open(
        "Images\\download.png").resize((25, 25), Image.ANTIALIAS))
    download = customtkinter.CTkButton(master=frame3, image=download, text="", width=30, height=20, fg_color="transparent",
                                       hover_color="#1e1f1e", compound="left", command=lambda: download_attachment(content, filename))
...
def clientpageL(email, password):
    ...
    if flag_null == '0':
        ...
        welcome_label = customtkinter.CTkLabel(
            master=client, text="Welcome: "+email, font=('Century Gothic', 12))
        welcome_label.place(x=30, y=15)

        l2 = customtkinter.CTkLabel(
            master=client, text="Inbox", font=('Century Gothic', 20))
        l2.place(x=280, y=15)

        scrollable_frame = customtkinter.CTkScrollableFrame(
            client, width=300, height=320)
        ...
        compose = ImageTk.PhotoImage(Image.open(
            "Images\\composee.png").resize((40, 40), Image.ANTIALIAS))
        compose_btn = customtkinter.CTkButton(master=client, text="", image=compose, width=50, height=20,
                                              fg_color="transparent", hover_color="#1e1f1e", compound="left", command=lambda: send_email(email))
    ...
    if flag_null == '1':
        ...
        nomail = customtkinter.CTkLabel(
            master=nomail_frame, text="No Mails Available", font=('Century Gothic', 20))
...
def signup_invalid():
    ...
    l2 = customtkinter.CTkLabel(
        master=frame, text="Sign Up", font=('Century Gothic', 18))
    l2.place(x=125, y=15)

    l6 = customtkinter.CTkLabel(
        master=frame, text="INVALID", font=('Century Gothic', 18))
    l6.place(x=120, y=45)

    firstname_entry = customtkinter.CTkEntry(
        master=frame, width=220, placeholder_text='Firstname')
    firstname_entry.place(x=50, y=85)

    lastname_entry = customtkinter.CTkEntry(
        master=frame, width=220, placeholder_text='Lastname')
    lastname_entry.place(x=50, y=125)

    email_signup_gui = customtkinter.CTkEntry(
        master=frame, width=220, placeholder_text='Email')
    email_signup_gui.place(x=50, y=165)

    email_error = customtkinter.CTkLabel(
        master=frame, text="*Domain must be @litemail.com or Email already Exists", font=('Century Gothic', 10))
    email_error.place(x=50, y=195)

    password_signup_gui = customtkinter.CTkEntry(
        master=frame, width=220, placeholder_text='Password', show="*")
    password_signup_gui.place(x=50, y=225)

    password_error = customtkinter.CTkLabel(
        master=frame, text="*Password must be at least 8 characters", font=('Century Gothic', 10))
    password_error.place(x=50, y=255)

    signup_btn = customtkinter.CTkButton(master=frame, width=100, text="Sign Up", command=lambda: signup_action(
        email_signup_gui.get(), password_signup_gui.get()), corner_radius=6)
...
def signup():
    ...
    l2 = customtkinter.CTkLabel(
        master=frame, text="Sign Up", font=('Century Gothic', 20))
    l2.place(x=125, y=45)
    ...
    email_val = customtkinter.CTkLabel(
        master=frame, text="*Email must be @litemail.com", font=('Century Gothic', 10))
    ...
    password_val = customtkinter.CTkLabel(
        master=frame, text="*Password must be at least 8 characters", font=('Century Gothic', 10))
...
def homepage(user):
    if (user == "valid"):
        ...
        frame = customtkinter.CTkFrame(
            master=l1, width=320, height=360, corner_radius=20)
        frame.place(relx=0.5, rely=0.5, anchor=tkinter.CENTER)
        ...
        email_login_gui = customtkinter.CTkEntry(
            master=frame, width=220, placeholder_text='Email')
        email_login_gui.place(x=50, y=110)

        password_login_gui = customtkinter.CTkEntry(
            master=frame, width=220, placeholder_text='Password', show="*")
        ...
        sign_btn = customtkinter.CTkButton(master=frame, width=100, text="Sign Up Now", command=lambda: signup(
        ), corner_radius=6, hover_color="#252625")
    ...
    if (user == "invalid"):
        ...
        error = customtkinter.CTkLabel(
            master=frame, text="Invalid Email or Password", font=('Century Gothic', 10))
